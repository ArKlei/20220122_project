...
<a href="{{route('student.index')}}" id="student">Students</a>
  ...
  <a href="{{route('school.create')}}" id="create_school">Add School</a>
    ...
    <div class="container">
        <p><h1 style="text-align:center; font-size:50px; color:gold"> {{$student->name}} {{$student->surname}}  </h1>
        <p>Id : {{$student->id}}</p>
        <p>Name : {{$student->name}}</p>
        <p>Surname : {{$student->surname}}</p>
        <p>Group ID : {{$student->group_id}}</p>
        <p>Image :<br><img src="{{$student->image_url}}" style="width:150px; height:auto" alt="{{$student->name}}"></p>
        ...
        <p><a class="btn btn-secondary" style="width:100px" href="{{route('student.index')}}">Back</a>
    </div>
